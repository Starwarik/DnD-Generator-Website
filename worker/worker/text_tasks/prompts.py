...
adventure_info_prompt = """Сгенерируй описание локации для ролевой настольной игры D&D (Dungeons & Dragons) пятой редакции.
Локация: "{nameLocation}", жанр: "{nameSetting}", количество участников: {playerNum}. Помести описание в объект "location" в формате JSON, содержащий ключи: "name" — название локации;
"description_location" - краткое описание локации в двух предложениях; "description_places" — текстовое описание ключевых точек локации перечислены по номерам.
Общее содержание описания локации и объектов не должно привышать 500 токенов"""

adventure_info_answer = "{description_json_answer}"

items_prompt = """Сгенерируй в формате JSON описание возможных сокровищ (не более {playerNumXthree} предметов), которые могут быть найдены на территории ранее описанной локации (объект "location"). 
Используй описания сокровищ из руководства D&D пятой редакции. Структурируй результат отдельно в объекте "items", без включения описания самой локации. 
...
который прописан как в D&D. Колличество токенов для описания одного сокровища не должно привышать 250 токкенов"""

items_answer = "{items_json_answer}"

npcs_prompt = """Сгенерируй в формате JSON описание внутриигровых персонажей D&D (не более {playerNumXthree}), которые находятся на территории локации, описанной в объекте "location".  
...
Результат помести в отдельный объект "npc". Колличество токенов для описания одного персонажа не должно привышать 250 токкенов"""
