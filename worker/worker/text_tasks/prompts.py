...
items_regeneration_concrete_prompt = Message(type=MessageType.user, content="""Создай новый предмет вместо "{nameItem}", используя описание локации (жанр "{nameSetting}"). Структурируй в объект "items" с ключами: "name", "description", "values", "type", "damage" (только для оружия), "armor_class" (только для брони). Общее описание не должно превышать 500 символов""")

npcs_regeneration_prompt = Message(type=MessageType.user, content="""Сгенерируй {playerNum} - 4 новых NPC на основе локации (жанр "{nameSetting}"). Привяжи их к квестам и предметам. Объект "npc" содержит ключи: "name" — имя; "disc_costum" — описание внешности; "dic_life" — образ жизни и роль. Общее описание не должно превышать 500 символов""")

npcs_regeneration_concrete_prompt = Message(type=MessageType.user, content="""Сгенерируй нового NPC вместо "{nameNPC}" на основе локации (жанр "{nameSetting}"). Учти расу, характер и поведение. Привяжи его к квесту и предметам. Объект "npc" содержит ключи: "name" — имя; "disc_costum" — описание внешности; "dic_life" — образ жизни. Общее описание не должно превышать 500 символов""")

quests_regeneration_prompt = Message(type=MessageType.user, content="""Сгенерируй {playerNum} - 4 новых квестов, используя описание локации (в соответствии с жанром "{nameSetting}"). 
Привяжи их к ранее созданным персонажам и предметам. Помести квесты в объект "quests" с ключами: "name" – название, "description" – описание, "goal" – цель, "rewards" — объект с наградами, включающий монеты (gold, silver, bronze). Общее содержание ответа не должно привышать 500 символов""")

quests_regeneration_concrete_prompt = Message(type=MessageType.user, content="""Сгенерируй новый квест вместо "{name_quest}", используя описание локации (в соответствии с жанром "{nameSetting}"), с новым персонажем и наградами. 
Помести в объект "quests" с ключами: "name" – название, "description" – описание, "goal" – цель, "rewards" — объект с наградами, включающий монеты (gold, silver, bronze). Общее содержание ответа не должно привышать 500 символов""")
