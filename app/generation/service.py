...
def generate_images_adventure(adventure: Adventure, session: Session):
...
def generate_images_characters(adventure: Adventure, session: Session):
    content = AdventureInfo.model_validate_json(adventure.content)
...
def generate_images_items(adventure: Adventure, session: Session):
    content = AdventureInfo.model_validate_json(adventure.content)
...
def generate_adventure_with_models(
    ...
    for tries in range(5):
        out_try, messages = _generate_iterative_chat(instructions[len(out):], extra_info, messages)
        ...
        if len(out) == len(instructions):
            ...
            #generate_images_adventure(adventure, session)
